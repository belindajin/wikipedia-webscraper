...
public class WikiScraper {
	...
	public WikiScraper(String start, String end, int size, double random) {
		...
		while (counter <= size) {
			...
			Pattern p = Pattern.compile("<div id=\"mw-content-text\" lang=\"en\" dir=\"ltr\" "
					+ "class=\"mw-content-ltr\">(.*)"); // matching URL to another Wikipedia page
			Matcher m = p.matcher(contents);
			
			if (m.find()) {
				Pattern p2 = Pattern.compile("/wiki/(.*?)\""); //matching page name from URL
	...
	/**
	 * 
	 * @return true if end vertex is reached, false otherwise
	 */
